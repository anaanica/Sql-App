    ...
    public partial class MainForm : Form
    {
        ...
        private void BtnExecuteQuery_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(tbContent.Text.Trim()))
            {
                ...
                dgQueryResults.DataSource = RepositoryFactory.GetRepository()
                    .ExecuteQuery(tbContent.Text.Trim(), OnInfoMessageGenerated, OnStatementCompleted);
            }
        }

        private void OnInfoMessageGenerated(object sender, SqlInfoMessageEventArgs e)
        {
            tbMessages.Text += $"{e.Message}" + Environment.NewLine;
        }

        private void OnStatementCompleted(object sender, StatementCompletedEventArgs e)
        {
            tbMessages.Text += $"({e.RecordCount} row/s affected)" + Environment.NewLine +
               Environment.NewLine + $"Completion time: {DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}"
